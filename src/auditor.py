...
class IngestAuditor:
    ...
    def print_summary(self, r: dict):
        s = r["summary"]
        print(f"\n{'='*60}")
        print(f"  3GPP Index Audit Report")
        print(f"  Time: {r['audit_time']}")
        print(f"{'='*60}")
        print(f"  Disk files: {s['total_disk_files']}  |  Manifest: {s['total_manifest_entries']}")
        print(f"  ChromaDB chunks: {s['total_chroma_chunks']}  |  Sources: {s['total_chroma_sources']}")
        print(f"  BM25 entries: {s['bm25_entries']}")
        print(f"{'='*60}")
        print(f"  [OK] Processed:   {s['processed']}")
        print(f"  [..] Pending:     {s['pending']}")
        print(f"  [!!] Failed:      {s['failed']}")
        print(f"  [==] Dup TDoc:    {s['duplicate_tdocs']}")
